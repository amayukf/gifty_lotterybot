...
export async function markRoundAsDrawn(roundId, winnerTicket, winnerUserId) {
  await db.update(schema.lotteryRounds).set({ status: 'drawn', winnerTicket, winnerUserId, updatedAt: new Date().toISOString() }).where(eq(schema.lotteryRounds.id, roundId)).run();
}
